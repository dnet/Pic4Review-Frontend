...
describe("Model > Mission", () => {
	describe("Constructor", () => {
		it("creates the object", () => {
			const m1 = new Mission("integrate", "amenity", G1, AREA, DESC);
		...
		it("fails if no type is defined", () => {
			assert.throws(() => { new Mission(null, "amenity", G1, AREA, DESC); }, TypeError);
		});
		
		it("fails if type is invalid", () => {
			assert.throws(() => { new Mission("not a valid type", "amenity", G1, AREA, DESC); }, TypeError);
		});
		
		it("fails if no theme is defined", () => {
			assert.throws(() => { new Mission("fix", null, G1, AREA, DESC); }, TypeError);
		});
		
		it("fails if theme is invalid", () => {
			assert.throws(() => { new Mission("fix", "not a valid theme", G1, AREA, DESC); }, TypeError);
		});
		
		it("fails if dataset is invalid", () => {
			assert.throws(() => { new Mission("fix", "amenity", {}, AREA, DESC); }, TypeError);
		});
		...
		it("fails if no short description is defined", () => {
			assert.throws(() => { new Mission("fix", "amenity", G1, AREA, {}); }, TypeError);
		});
	});
